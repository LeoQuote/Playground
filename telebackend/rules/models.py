...
class TeleUser(models.Model):
    ...
    is_bot = models.BooleanField('是机器人吗?')
    json_data = models.CharField('json data', max_length=1024)
    status = models.CharField('现在所处模式', max_length=32)
    status_since = models.DateTimeField('进入模式时间')
...
class Rule(models.Model):
    ...
    response_text = models.CharField('响应文本', max_length=128)
    been_triggered_for = models.IntegerField('触发次数')
    last_triggered_at = models.DateTimeField('上次触发时间')
